...
struct ContentView: View {
    ...
    private func subtitle(for item: Game) -> String {
        var parts: [String] = []
        
        parts.append("Players: " + describeField(item.players))
        parts.append("LastPlayed: " + describeField(item.lastPlayed))
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Stepper("Players: \(draft.players)", value: $draft.players, in: 0...100000)
                    .accessibilityIdentifier("field_players")
                DatePicker("LastPlayed", selection: $draft.lastPlayed, displayedComponents: .date)
                    .accessibilityIdentifier("field_lastPlayed")
